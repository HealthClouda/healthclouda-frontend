...
| @Bastoh | **Admissions — medical advisor's real answers (Q2/Q3), correcting #139** | `fix/admissions-medical-answers-ui` (stacked on `feat/ward-admissions-write-path`, PR #139, unmerged) | `src/components/dashboard/nurse/NurseDashboard.tsx`, `.test.tsx`, `CODEBASE_FLAGS.md` | 2026-09-13 | 🟡 **Q3: AMA reason must NOT be required (advisor: "sometimes there might be no particular reason"); AMA is signed by a doctor, not a free-text witness. Q2: on-duty must gate the doctor picker, not just sort it.** #139 predates the advisor's answers and built the opposite on both. No new endpoint assumed — degrades against the current schema; a real `attending_doctor` FK/handover-history gap is flagged, not invented |
