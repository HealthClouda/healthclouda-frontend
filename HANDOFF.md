...
|---|---|---|---|---|---|
| @Bastoh | **Receptionist check-in write path** — POST `/receptionist/check-ins/` (create) + PATCH `/receptionist/check-ins/<id>/` (queue status/doctor). `REC_CHECK_INS` today appears only in GET call sites — the receptionist can see the queue but cannot check anyone in | `fix/receptionist-checkin-write-path` | `src/components/dashboard/receptionist/ReceptionistDashboard.tsx`, its test file, `CODEBASE_FLAGS.md` | 2026-09-11 | 🟡 **claimed before cutting the branch** |
