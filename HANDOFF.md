...
| @Bastoh | **Doctor admissions page** — closes FLAG-040 (UI half) + FLAG-042, `GET /ward/admissions/?mine=true` | `feat/doctor-admissions-page` | `DoctorDashboard.tsx`, `types/dashboard.ts`, `CODEBASE_FLAGS.md` | 2026-09-15 | 🟡 **In progress.** Reserving **FLAG-241** for anything found while building. Depends on backend `?mine=true` — measured merged on backend `develop` (`apps/ward/models.py admissions_for_doctor`, `apps/ward/views.py`) at commit `7c84fa0`, not a separate open PR |
