...
| @Bastoh | @Qeeyat | **#130 has no reviewer set and no In Flight row** — the doctor create-referral wedge feature | `CLAUDE.md` §3: a @Bastoh PR is reviewed by @Qeeyat, assigned **when it is opened**, and the row is claimed **before** the branch is cut | 🟢 Not blocking — I have seen it and will review it; raised so the mechanism is not quietly skipped on the PR that carries the demo wedge feature | 2026-09-08 | ✅ **CLEARED 2026-09-14.** Reviewer set, late In Flight row raised on #132, reviewed and approved by @Qeeyat, **#130 merged** (`d0751d4`). #132 should now be closed, not merged: its row blocks on a P1 fixed 8 Sep |
...
| @Qeeyat | @Bastoh | **The review bottleneck has reversed direction, and I have started multiplying it.** Your 7–8 Sep round emptied your side of the board; the queue is now entirely **mine** — **#130, #132, #134**, all green and MERGEABLE, plus whatever the build agents open next. **You already established why this is structural** (`bypass_actors: []`, *"verified from an admin account: admin does not help"*, row 3 of *Do these first*) — I am not re-reporting that, I am reporting that I am now feeding it: **I am running frontend build agents, and they author as me**, so every PR they produce needs you. **Not a nudge on #130** — your own row above already covers it. The ask is the shape: take the queue when you can, **or** tell me to add a bypass actor and I will. If neither, say so and I will throttle the agents instead of stacking work you did not ask for. | The approval is yours by construction, and the bypass is a repo-settings call I will not make unilaterally | 🟠 **Nothing is blocked on code — it is blocked on merge**, which is the actual bottleneck. Finished work piling up unreviewed is the failure mode this table exists to surface | 2026-09-11 | ✅ **ANSWERED 2026-09-14 by @Qeeyat: take the queue + throttle the agents; no bypass actor.** Ruleset `11328360`'s one approval is the only enforced gate, CI is not a required check, and stacked PRs run no CI (FLAG-230), so a bypass lets agent-authored code reach `develop` with nobody and nothing checking it. Better uses of repo settings: required status checks, plus `ci.yml` `branches: ['**']`. Queue taken the same day: all 11 reviewed, #130/#135/#145 approved and merged. The claim-PR churn is FLAG-240 |
| 🔌 backend | @Qeeyat | **The ordered admission path (`admission-requests/{id}/accept/`) skips the ward gender policy** — no check, no `override`, no audit; the direct path has all three. **Backend #192** · FLAG-239 | Their serializer/service — `admit_patient()` is the shared path the rule never moved into | 🟠 Not blocking a merge, but #139's accept two-step is dead code until answered, and a nurse accepting an order can place a patient against ward policy unaudited | 2026-09-14 | ✅ **CLEARED 2026-09-14, same day.** Backend #192 closed by backend **#193** (`12eb32c`): the gender check moved into the shared `admit_patient()` and `override` added to accept. Backend **#194** put the error under `details.gender`. Frontend consumed it in **#139** (`5262e64`, merged `2671a8f`). FLAG-239 resolved |
| @Bastoh | @Qeeyat | **Your agents read one shared backend checkout while another agent had uncommitted changes in it.** #150's withdrawn FLAG-241 records the result: "`?mine=true` is merged" was read off a working tree and put into #149's claim row as fact. Worth a rule for your agents: one checkout (or worktree) per agent, and claim "merged" only from `git show origin/develop:<path>` | Your agent setup, your lane | 🟠 Not blocking, but a false "merged" claim nearly let #150 ship a doctor page showing **every admission in the org**. Backend #207 has since merged (2026-09-15 14:48, `baf2c8b`), so that exposure is closed | 2026-09-15 | **OPEN** |
