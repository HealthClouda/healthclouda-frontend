...
| @Bastoh | **FLAG-024** — repair the stale e2e specs | `fix/flag-024-stale-e2e-specs` | `e2e/auth.spec.ts`, `e2e/landing.spec.ts`, `playwright.config.ts`, `CODEBASE_FLAGS.md`, `HANDOFF.md` | 2026-09-01 | 🟡 **PR #118 — change request addressed 2026-09-02, awaiting @Qeeyat re-review.** The `mobile` (Pixel 5) project is **removed**, not silenced: it ran the two desktop-shaped suites at 393px and, once those were ignored, would have run zero tests under a name implying coverage. Small-viewport behaviour stays with `e2e/design/smallscreen.spec.ts` at explicit breakpoints. Credential-free specs **13/13, exit 0**; the design specs cannot run without `E2E_*` credentials (B8) |
