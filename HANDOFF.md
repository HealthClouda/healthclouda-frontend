...
|---|---|---|---|---|---|
| @Qeeyat | **In Flight + Cross-Lane cleanup** after six merges (7–8 Sep) | `docs/clear-in-flight-2026-09-08` | `HANDOFF.md` | 2026-09-08 | 🟡 **this PR.** Docs only. Six rows below described work that merged on 7–8 Sep; the table's own rule is that a stale claim is worse than no claim |
| @Qeeyat | **T3 — the role-gate & tenant-isolation suite** (`BETA_READINESS.md` Tier 1, item 4) | `test/t3-role-gate-isolation` | `src/**/__tests__` (new), `middleware.test.ts`, `CODEBASE_FLAGS.md` | 2026-09-08 | 🟡 **claimed before cutting the branch.** The largest unbuilt Tier-1 item: A5/FLAG-001's fix is unfalsifiable without it. **Done when at least one test has been proven able to fail** against a deliberately weakened gate |
...
| @Bastoh | **Doctor "create referral" UI** — the wedge feature had no create path at all | `feat/doctor-referral-create` | `DoctorDashboard.tsx`, `lib/config.ts`, `types/dashboard.ts`, `CODEBASE_FLAGS.md` | 2026-09-08 | 🔴 **PR #130 — DON'T MERGE, review found a P1** (state not reset between patients — one patient's clinical narrative/consent can submit under another's name). This row is late — it should have been committed to `develop` before the branch was cut, and was not; correcting that now rather than leaving it invisible until merge. Fixing the P1, its `NewEpisodePanel` sibling, and two P2s in this same PR |
