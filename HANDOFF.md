...
|---|---|---|---|---|---|
| @Qeeyat | **FLAG-235** — a screenshot baseline encodes a day counter, so the nurse design test fails worse every day | `fix/flag-235-nurse-baseline-drift` | `CODEBASE_FLAGS.md`, `HANDOFF.md` (+ `e2e/design/roles.spec.ts` and one baseline if the fix lands here) | 2026-09-09 | 🟡 **Flag logged; fix not yet written.** Diagnosed: `length_of_stay` renders as `(13d)` and `masksFor()` does not match it. **Nurse is the only view passing `admittedAsDate: true`**, which is why doctor passes on the same run |
