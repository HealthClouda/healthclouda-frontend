...
| 3 | **Review #126, #128 and #109** (@Bastoh) | Both are @Qeeyat's and blocked purely on review. Ruleset `bypass_actors: []` — **verified from an admin account: admin does not help.** #128 carries the Nurse render and FLAG-229/230/231/232; **#109 closes the whole stat-tile class — 9 phantom fields to 0, all six dashboards clean.** ⛓️ **Merge #128 before #109**: #109 comments cite FLAG-231/232, whose entries live on #128. And once both land, delete `knownStatBug: 'FLAG-231'` from `roles.spec.ts` or that test XPASSes and goes red — the mechanism working, but only if you expect it |
