...
|---|---|---|---|---|---|
| @Bastoh | **FLAG-041** — surface the attending doctor as a handover on the nurse dashboard, using the already-nurse-readable `/ward/admissions/` endpoint (not a backend change) + **FLAG-211** correction (nurse admission permission, unverifiable from the schema, is confirmed granted by reading `apps/core/permissions.py` on the backend) | `fix/flag-041-attending-doctor-handover` | `src/types/dashboard.ts`, `src/components/dashboard/nurse/NurseDashboard.tsx` (+ its test file), `CODEBASE_FLAGS.md` | 2026-09-13 | 🟡 In progress |
