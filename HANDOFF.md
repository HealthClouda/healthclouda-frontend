...
|---|---|---|---|---|---|
| @Qeeyat | **FLAG-235 escalation + the 9 Sep session landed** — the mask defect, and the first-ever Patient render | `docs/land-2026-09-09-session` | `CODEBASE_FLAGS.md`, `HANDOFF.md`, `HANDOFF-Qeeyat.md` | 2026-09-13 | 🟡 **Docs only — the mask fix itself is NOT in this branch.** Lands work that sat uncommitted on a laptop from 9 Sep to 13 Sep: FLAG-235 re-diagnosed (**two of three content masks match strings the app never emits**), and **the Patient dashboard was rendered for the first time on 2026-09-09**. The five Patient baselines stay **uncommitted on purpose** — see the flag |
| @Qeeyat | **T6 — `docs/UAT-CHECKLIST-FE.md`**, the frontend role-journey scripts. Owed since week 2 and never written; the prep for next week's daily page-by-page testing | `docs/t6-uat-checklist-fe` | `docs/UAT-CHECKLIST-FE.md` (new) | 2026-09-13 | 🟡 **Being written.** Mirrors the backend's J1–J7 **through the UI** — theirs is API-level `curl`. **No credentials in the file: this repo is public** |
...
| @Bastoh | **#137** — receptionist check-in write path, addressing @Qeeyat's review | `fix/receptionist-checkin-write-path` | `ReceptionistDashboard.tsx` + test, `CODEBASE_FLAGS.md` | 2026-09-14 | 🟡 **Pushed for re-review.** P1 (blank `reason_for_visit` 400) fixed, watched RED→GREEN; `readableFieldError` now reads the real `details` envelope instead of returning `null` on every response; FLAG-027 renumbered to FLAG-030 (collided with #130's, already merged) |
| @Bastoh | **#139** — ward admissions write path, addressing @Qeeyat's two P1s | `feat/ward-admissions-write-path` | `NurseDashboard.tsx`, `DoctorDashboard.tsx` + tests, `CODEBASE_FLAGS.md` | 2026-09-14 | 🟡 **Pushed for re-review.** Rebased through #130/#145's conflicts. Nurse reassign button removed (403 for any non-DOCTOR, per the owner) with a negative-control test in its place; `clinical_reason`/`override` re-verified live on backend `develop` (not taken on trust); new **FLAG-031** — the gender two-step's 400 carries no field name on any of the three admit surfaces, worked around with a documented text match, real fix is backend-side |
| @Bastoh | **#142** — admissions medical-answer corrections, addressing @Qeeyat's review | `fix/admissions-medical-answers-ui` (stacked on #139) | `NurseDashboard.tsx` + test, `CODEBASE_FLAGS.md`, `HANDOFF.md` | 2026-09-14 | 🟡 **Pushed for re-review.** Off-duty attending doctor is now selectable (not `disabled`) with the same select→warn→"Assign anyway" two-step as the gender check, matching the backend's warn-and-allow policy — watched RED→GREEN. Its own FLAG-041 dropped (superseded by #145, merged first, per @Qeeyat's own instruction); stale FLAG-041→042 pointers fixed |
...
> absent. Kept here because that misread cost eleven days and the shape is still wrong in the schema.
...
*Cleared on merge — **2026-09-07/08, six PRs, and it emptied the board**: **#109** FLAG-222/227/231 stat tiles (9 phantom fields → 0 across all six dashboards) · **#107** issue #101 HCL-ID handout · **#126** the #99/#100 In Flight clear + FLAG-228 · **#128** T5 Nurse rendered + Patient wired (FLAG-229/230/231) — all four @Qeeyat's, **all four had been finished and were blocked on review only**, and @Bastoh cleared every one · **#118** FLAG-024 stale e2e specs · **#127** his 3 Sep session log — both rebased out of their CONFLICTING state and merged. ⚠️ **#126 was rebased before merge, so its four commits are on `develop` under new SHAs (`49b453d`)** — a local branch that still holds the originals looks unmerged and is not. 🎯 **After this round the only open PR in the repo is #98**, and the only In Flight rows that are not mine are his two held items.*

*Cleared on merge: **#99** A5/FLAG-001 server-trusted auth · **#100** FLAG-210 patient portal (both merged 2026-09-03 by @Qeeyat, locally, with the `CODEBASE_FLAGS.md` conflict resolved additively in flag-number order — both sides kept, proven by a heading diff against each parent) · **#109** FLAG-222 Superadmin stat tiles + FLAG-227/231 folded in · **#107** Issue #101 HCL-ID handout at registration (both merged 2026-09-07 by @Bastoh, branches deleted; **#109** gained one more commit before merging, `c610456`, correcting `users_trend`/`orgs_trend` from `number | null` to `string | null` — the backend sends a formatted string) · **#111** `CLAUDE.md` §3 branch-protection correction + FLAG-021 — **merged 2026-09-02 by @Bastoh as a merge commit** (`88ae5ad`) after a rebase off a 31-commit-stale base; `develop` after it: **tsc clean · lint clean · 192/192 · build green**. · **#114** In Flight cleanup + session log · **#115** T5 pass + FLAG-227 (both merged 2026-09-01 by @Qeeyat) · **#96** schema-reading guidance — **CLOSED 2026-08-31, not merged.** ⚠️ **Corrected 2026-09-06:** this previously read *"fixed at source instead"*, which is misleading — **none of its content is on any merged branch** (verified on `develop`: no unauthenticated-schema bullet in `CLAUDE.md`, no "Four habits" in `ONBOARDING.md`, no FLAG-217 correction). The branch is rebased and re-verified and needs reopening or a written abandonment — **FLAG-234**. · Earlier: **#116** E8/FLAG-006 CI + ESLint gating · **#117** B7 coverage · **#113** + **#119** @Bastoh session logs · **#112** stale-doc sweep (ARCH-7 closed) — all five **reviewed and merged 2026-09-01 by @Qeeyat**, as merge commits. `develop` after all five: **tsc clean · lint clean (the new gate) · 192/192 · build green · middleware 35.4 kB**. Earlier the same round: **#106** FLAG-203/221 small-screen gate · **#110** FLAG-223/224/225/226 Patient contract audit · **#108** Gate 2 assessment · **#105** session log 29 Aug — merged 2026-08-31, each trial-merged and verified locally before merging rather than from the PR body (**192/192**). **#106 closed the worst PHI exposure in the repo** — staff dashboards no longer mount, fetch, or put records in the DOM below 768px. · **#97** In Flight cleanup + FLAG-017/018/019 · **#102** `SECURITY_BASELINE.md` (A7) · **#103** E7/FLAG-005 server-fetch observability · **#104** FLAG-220 Org Admin referrals page — all four **reviewed and merged 2026-08-30 by @Qeeyat**, as merge commits, each re-verified locally before merging rather than from the PR body (`develop` after all four: tsc clean, **171/171**, build green, middleware 35.4 kB). #104 closes the referral hole FLAG-220 described — the journey can now be accepted. · **A2/A3/A4/A6** Tier-1 infra batch — PR #65 · **FLAG-010** — PR #66 · **FLAG-011
...
| 🔌 backend | @Bastoh | **Does `api-beta` exist?** Both `api-beta.healthclouda.com` and `beta.healthclouda.com` were **NXDOMAIN** on 2026-09-01 | Their infra | 🔴 **Blocks the beta promotion.** We will not point `beta.` at `api-dev` — that is the tier-crossing that presents as *"the invite is broken"* | 2026-09-01 | 🔴 **RE-MEASURED 2026-09-13 (@Qeeyat) — the backend half is DONE and the remaining half is OURS.** `api-beta.healthclouda.com/api/v1/schema/` → **200** (resolves 104.21.34.204, cert valid), unchanged since 7 Sep. **`beta.healthclouda.com` still NXDOMAIN** — measured against `1.1.1.1` with a nonsense control returning NXDOMAIN, so the resolver was honest. 🎯 **This is no longer a backend ask at all: it is one DNS record on Vercel/Cloudflare, `[INFRA]`, @Bastoh.** It is the sole remaining blocker on #98 (held 16 days) and on Gate 2 criterion 2 |
| 🔌 backend | @Qeeyat | **`api-beta` needs a TLS certificate covering its own hostname** — it resolves and listens, but `:443` fails `SEC_E_WRONG_PRINCIPAL` | Their infra; nothing on our side can work around a cert that does not match | 🔴 **Blocks #98, T9 and Gate 2 criterion 2.** And it now *looks* provisioned, so the risk is someone setting the `staging` override on the strength of DNS alone | 2026-09-05 | ✅ **RESOLVED 2026-09-07, re-measured by @Qeeyat 2026-09-08:** `https://api-beta.healthclouda.com/api/v1/schema/` returns **200**, not `SEC_E_WRONG_PRINCIPAL`. The certificate now covers the name. 🎯 **The `BETA_READINESS.md` item 3 precondition is satisfied** — #98 is held on DNS alone now, not on TLS |
...
| @Bastoh | @Qeeyat | **Send `E2E_PATIENT_EMAIL` / `E2E_PATIENT_PASSWORD`** out-of-band (never into this repo — **it is public**) | Credentials only he holds | 🔴 **Patient is the ONLY dashboard nobody has ever rendered.** Also confirms **FLAG-231** and settles **FLAG-226**, the candidate P0 in `BETA_READINESS.md`'s own Tier 1 | 2026-09-04 | ✅ **DONE — cleared 2026-09-13 by the raiser.** The credentials arrived and were used: **five Patient baselines were captured 2026-09-09** (`patient-overview` desktop + mobile, `my-health`, `appointments`, `access-referrals`), which the harness cannot produce without a successful patient sign-in — it skips cleanly when the vars are absent. ⚠️ Inferred from the captured artifacts, not from reading `.env.local`. **Patient is no longer the dashboard nobody has rendered** |
| @Bastoh | @Qeeyat | **Rebase #127 and #118** — both approved, both now CONFLICTING | His branches; force-pushing another dev's branch is not something to do unattended | 🟠 Both are approved and ready otherwise. #118 also wants a longer timeout before it goes into CI — it is flaky on a **cold** `next dev` | 2026-09-04 | ✅ **DONE 2026-09-07.** Both rebased out of CONFLICTING and merged — #118 (`aa5f435`) and #127 (`ba7ed62`). ⚠️ #118's cold-`next dev` flakiness is **not** fixed by the rebase and is still live for whenever CI runs it — FLAG-024's own note |
...
| @Bastoh | @Qeeyat | **Re-review #109 (FLAG-222/227/231)** — the work has been finished since 2026-09-06 | His change request from **30 Aug** stands, and only he can clear it | 🔴 **Blocked on review, not on work.** MERGEABLE, all CI green, all three dashboards folded in. It is the fix for the bug class that has hit **four** dashboards, still unmerged while PHI lands | 2026-09-06 | ✅ **CLEARED — #109 merged 2026-09-07 by @Bastoh**, branch deleted |
| @Bastoh | @Qeeyat | **Re-review #107 (issue #101)** — all three change requests addressed 2026-09-06 | Same: his standing CHANGES_REQUESTED | 🟠 The HCL-ID handout is the **first screen of the receptionist journey**. Open **8 days**, and the branch was 71 commits behind before today's rebase | 2026-09-06 | ✅ **CLEARED — #107 merged 2026-09-07 by @Bastoh**, branch deleted |
| @Bastoh | @Qeeyat | **#96 — did we mean to close it?** I closed it 31 Aug and recorded no reason; its content is on **no merged branch** | Shared call: you reviewed it, verified it yourself and called it *"right, valuable"* | 🟠 Not blocking anyone, but the guidance a new dev needs on the contract seam is currently unreachable. Reopen + re-review, or agree to abandon it **in writing** — **FLAG-234** | 2026-09-06 | **OPEN** |
| @Bastoh | @Qeeyat | **#130 has no reviewer set and no In Flight row** — the doctor create-referral wedge feature | `CLAUDE.md` §3: a @Bastoh PR is reviewed by @Qeeyat, assigned **when it is opened**, and the row is claimed **before** the branch is cut | 🟢 Not blocking — I have seen it and will review it; raised so the mechanism is not quietly skipped on the PR that carries the demo wedge feature | 2026-09-08 | **OPEN** |
| @Qeeyat | @Qeeyat | ⚠️ **I told you on #96 that you had blocked it nine days. That was wrong** — corrected on the PR the same hour | My error, recorded rather than quietly dropped | 🟢 Nothing to do — noted so it is not learned as fact from the thread | 2026-09-06 | ✅ **DONE** |
| @Qeeyat | @Bastoh | **The review bottleneck has reversed direction, and I have started multiplying it.** Your 7–8 Sep round emptied your side of the board; the queue is now entirely **mine** — **#130, #132, #134**, all green and MERGEABLE, plus whatever the build agents open next. **You already established why this is structural** (`bypass_actors: []`, *"verified from an admin account: admin does not help"*, row 3 of *Do these first*) — I am not re-reporting that, I am reporting that I am now feeding it: **I am running frontend build agents, and they author as me**, so every PR they produce needs you. **Not a nudge on #130** — your own row above already covers it. The ask is the shape: take the queue when you can, **or** tell me to add a bypass actor and I will. If neither, say so and I will throttle the agents instead of stacking work you did not ask for. | The approval is yours by construction, and the bypass is a repo-settings call I will not make unilaterally | 🟠 **Nothing is blocked on code — it is blocked on merge**, which is the actual bottleneck. Finished work piling up unreviewed is the failure mode this table exists to surface | 2026-09-11 | **OPEN** |
| 🔌 backend | @Bastoh | **Decide FLAG-040** — should a DOCTOR get `GET /ward/beds/` (currently `CanManageWard`, which excludes DOCTOR), or is admit deliberately nurse-mediated? | `CanManageAdmissions` already trusts a DOCTOR to `POST /ward/admissions/`, but `CanManageWard` blocks the only endpoint that lists which beds are free — the two rules disagree on this one role | 🟡 Not blocking WARD-1 (shipped nurse-only) — but a doctor-side admit screen cannot be built either way until this is answered | 2026-09-11 | **OPEN** |
| 🔌 backend | @Bastoh | **Expose `attending_doctor` on `GET /nurse/my-patients/`** (`ActiveAdmissionSerializer`, `apps/ward/nurse_serializers.py:60-75`) — FLAG-041 | The advisor's Q2 answer requires the handover to be **visible**, not just recorded server-side; there is no field for the frontend to render it from today | 🟡 Not blocking the Q2/Q3 UI correction (`fix/admissions-medical-answers-ui`) — the picker and signer are fixed either way — but a persistent "who is attending" / handover trail cannot exist in this UI without it | 2026-09-13 | ✅ **CLEARED 2026-09-14 — no backend change needed.** #145 (merged) solved it a different way: `GET /ward/admissions/` already carries `attending_doctor`/`attending_doctor_name`/`needs_attending_doctor` on the same rows, so the frontend now fetches that alongside `/nurse/my-patients/` and merges by id. See the resolved FLAG-041 entry in `CODEBASE_FLAGS.md` |
...
> ✅ **2026-09-08 — the inbox worked, and it is worth saying so once.** @Bastoh **cleared his own two
> re-review rows** (#109, #107) rather than leaving the raiser to do it, which is the rule as written and
> the opposite of the 4 Sep deviation above. Both PRs merged 7 Sep. The three rows I updated today
> (**rebase #127/#118**, and the two `api-beta` rows) I updated as the **raiser**, on the same grounds as
> 4 Sep: all three are verifiable from outside the row — two merge commits on `develop`, and an HTTP code
> I re-measured myself.
>
> 🔴 **What is genuinely still open on @Bastoh, after the biggest merge round this repo has had:** the
> **`E2E_PATIENT_*` credentials**, the **required status checks** on ruleset 11328360, the **one-line
> `ci.yml` `branches: ['**']` fix** (FLAG-230), **#98**, and **#96** (FLAG-234). 🎯 **Four of those five
> are unchanged since 1–6 Sep, and none of them is work — they are a message, two settings, a merge
> button and a decision.** The patient credentials are the cheapest: one message closes
> `BETA_READINESS.md` Tier 1 items **2 and 6**, and confirms **FLAG-231**.
...
> 🔬 **Criterion 2 re-measured 2026-09-05 (@Qeeyat) — it is still ❌, and now for a sharper reason.**
> The phrase *"pending only the `api-beta` variable"* has quietly stopped being accurate. `api-beta`
> is no longer missing — it is **half-provisioned**: it resolves, both ports are open, `:80` redirects
> to HTTPS, and **`:443` fails TLS because the certificate does not cover the hostname**
> (`SEC_E_WRONG_PRINCIPAL`). `beta.healthclouda.com` is still NXDOMAIN.
>
> **So setting the variable is no longer sufficient, and doing it now would make things worse, not
> better** — `staging` would point at a host that answers and then fails every request. The criterion
> needs a precondition it never had: **`curl https://api-beta.healthclouda.com/api/v1/schema/` returns
> 200 first.** Full evidence in the 2026-09-05 contract notes below. **The verdict still has not been
> re-run in full; this is one criterion, measured.**
...
|---|---|---|
| 1 | ~~**Send `E2E_PATIENT_EMAIL` / `E2E_PATIENT_PASSWORD`**~~ | ✅ **DONE — they arrived, and Patient was rendered for the first time on 2026-09-09.** Five baselines captured (desktop + mobile). ⚠️ **This did NOT close `BETA_READINESS.md` item 2**, which asks for *a patient signing in and reaching their dashboard* verified against the tier that matters — a harness render on `dev.` is the rehearsal, not the gate. **And the baselines are deliberately uncommitted** until FLAG-235's mask is fixed: `patient-overview-desktop` carries `12d ago` twice |
...
| 3 | ~~**Review #126, #128 and #109**~~ | ✅ **DONE — all three merged 2026-09-07/08 by @Bastoh.** ⚠️ The queue has since **reversed**: it is now @Bastoh's PRs (#130, #132, #134) waiting on @Qeeyat. See the last Cross-Lane row |
...
| `beta.healthclouda.com` | `staging` | `api-beta` | ❌ **still NXDOMAIN 2026-09-13** (measured against `1.1.1.1`, nonsense control clean). ✅ **`api-beta` is fully up** — schema returns **200**, certificate valid, since 7 Sep; the 5 Sep TLS trap is gone and the runbook precondition is satisfied. **What is missing is the `beta.` DNS record itself — ours, `[INFRA]`, not the backend's.** Order still: `staging`-scoped `NEXT_PUBLIC_API_URL=api-beta` override **first**, domain second |
...
| Date | Note | Status |
|---|---|---|
| 2026-09-05 | 🔴 **`api-beta` is half-provisioned, and the half that exists makes it LOOK ready — measured today, on onboarding day.** It was NXDOMAIN on 2 Sep; it now has an A record (`69.46.46.33`), both ports open, and `:80` issuing a `301` to `https://`. **But `:443` fails TLS: `SEC_E_WRONG_PRINCIPAL` — the certificate does not cover `api-beta.healthclouda.com`.** So nothing can talk to it: not a browser, not `serverFetch`, not an emailed invite link. `beta.healthclouda.com` itself is **still NXDOMAIN**, and so is `api.healthclouda.com`. 🪤 **Why this is worse than plain NXDOMAIN:** the beta runbook says *"set the `staging`-scoped `NEXT_PUBLIC_API_URL=api-beta` override FIRST, attach the domain SECOND"* — written when the risk was pointing `beta.` at the **dev** backend. Do that today and `staging` points at a host that resolves, listens, and then fails TLS on every request. **It presents as "the app is broken", not as a config error** — the same silent class the sprint plan named. ⛔ **#98 must stay held, and the runbook now needs a precondition: `curl https://api-beta.healthclouda.com/api/v1/schema/` must return 200 BEFORE the override is set.** | 🔴 **OPEN — @Bastoh / backend.** Blocks B4/#98, T9 and Gate 2 criterion 2 |
| 2026-09-05 | ⚠️ **Do not measure DNS for this project from a plain `nslookup`/`curl` on a hijacked resolver — it manufactures false positives.** On @Qeeyat's connection every name under `healthclouda.com` — including `nonsense-check-4471.healthclouda.com` — resolves to `102.88.158.7`. A first pass today therefore reported **all six tier hosts resolving**, which would have read as *"`beta.` is up, we are ready"* on onboarding morning. Re-queried against `1.1.1.1`, with a known-bad control name to prove the resolver was honest, the real answers are the ones in the row above. 🔑 **The hijack fabricates answers; it does not fabricate NXDOMAIN — so previous NXDOMAIN readings (2 Sep) were sound, and only "it resolves now" needs re-checking.** Always: `nslookup <host> 1.1.1.1`, plus a nonsense control. | ✅ **method recorded** |
| 2026-09-05 | ✅ **The `dev` tier is serving current `develop`, verified by behaviour rather than by a dashboard.** `https://dev.healthclouda.com/patient` returns **307 → `/signin`**, which only exists after #100 (merged 3 Sep) — so **#99 and #100 are live on `dev.`**, and middleware routes a logged-out visitor to the patient portal correctly. **A2 still holds**: `railway.app` appears **0** times in the served HTML. ⚠️ **The apex is still stale — FLAG-018 confirmed open:** `https://healthclouda.com/patient` returns **404**, i.e. a build predating #100. | ✅ dev verified · 🔴 FLAG-018 open |
