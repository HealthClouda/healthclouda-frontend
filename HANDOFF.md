...
| @Bastoh | Admission-refusal handling on the nurse admit screen (frontend half of "build 1") — `details.patient` hard stop, 409 bed-conflict refresh, revert the #139/FLAG-031 text-match workaround | `fix/admission-refusal-handling-nurse` | `src/components/dashboard/nurse/NurseDashboard.tsx` (+ its test file) | 2026-09-15 | 🟡 **In progress.** Depends on backend `fix/admission-refusal-shapes` (not yet merged) for the 409/`details.patient` shapes to be live |
