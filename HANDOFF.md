...
| @Bastoh | **WARD-1** — the admissions write path (admit; `ENDPOINTS.ADMISSIONS`/`ENDPOINTS.ADMISSION` had zero call sites outside `config.ts`) | `feat/ward-admissions-write-path` | `src/components/dashboard/nurse/NurseDashboard.tsx` (+ its test), `src/types/dashboard.ts`, `CODEBASE_FLAGS.md` | 2026-09-11 | 🟡 **In progress.** Nurse-only in this PR — a DOCTOR can POST an admission per `CanManageAdmissions` but has no read access to `/ward/beds/` (`CanManageWard` excludes DOCTOR), so a doctor-side admit UI cannot pick a bed; logging this as its own flag. Not touching the receptionist surface |
