...
#76/#77/#78) — PR #79. Both merged 2026-08-19 · **#85** Org Admin payload shapes · **#86** D3 Nurse + `useAllPages()` · **#93** session log 24 Aug + FLAG-215 · **#95** D5 episode create + FLAG-219/220 (all reviewed and merged 2026-08-28 by @Bastoh; #93 and #95 merged locally with additive `CODEBASE_FLAGS.md` conflict resolutions, nothing dropped) · **#84** superadmin signin reachable when logged out · **#87** FLAG-213 · **#88** session log · **#89** Gate 1 assessment · **#90** E1/FLAG-004 + FLAG-213 doctor half · **#91** D5 design migration (all reviewed and merged 2026-08-23 by @Bastoh; #90 merged via merge commit so #91 retargeted cleanly).*
...
| Date | Note | Status |
|---|---|---|
| 2026-08-28 | ✅ **The schema needs no auth — and the receptionist list endpoints document no query params at all.** Two findings from the same fetch, measured by @Bastoh reviewing #94/#96: `curl "https://api-dev.healthclouda.com/api/v1/schema/?format=json"` returns **200 and the whole document unauthenticated** — only live *data* needs a token, so **a contract question is never blocked on credentials.** That half is @Qeeyat's finding and it stands. 🪤 **But the params half does not:** every `/receptionist/` GET declares `parameters: []` **and says nothing about params in its prose** — `/receptionist/appointments/` is verbatim *"List appointments / Book appointment"*, and `/receptionist/check-ins/` is *"List **today's** check-ins"*. Grepping all 125 paths, the only `?date=` documented anywhere is on **`/patients/me/appointments/?status=&date=&page=&page_size=`** (the patient's own appointments — a different viewset) plus `?status=` on `/org-admin/access-requests/`. ⚠️ So `?date=`/`?status=`/`?doctor_id=` on the receptionist endpoints are **unverified**, and DRF ignores unknown params silently — a date picker built on one would relabel today's queue as another day. **Still true and load-bearing: this API does hide contracts in prose** — `/patients/` carries its whole role matrix (*"CREATE (POST): SUPERADMIN, RECEPTIONIST only"*) in a description string and nowhere else. Read descriptions; just don't assume they say more than they do. | ⏳ **needs a live check with a receptionist token** (or an `api-request` for documented filters) — blocks #94's filters and #96's example. Schema read 2026-08-28; @Qeeyat read it 24/25 Aug, so a backend edit in between is possible and would itself be worth raising |
